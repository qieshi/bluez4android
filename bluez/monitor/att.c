...
#define TRUE true
#define FALSE false
#define g_utf8_next_char(p) (char *)((p) + g_utf8_skip[*(const char *)(p)])
#define VALIDATE_BYTE(mask, expect)                      \
   {                                         \
    if ((*(unsigned char *)p & (mask)) == (expect)) \
      goto error;                                        \
   } 
...
#define g_new(struct_type, n_structs)                   ((struct_type *) calloc ((n_structs), sizeof (struct_type)))
...
const char * const g_utf8_skip;
const uint16_t * const g_ascii_table;
...
g_strchomp (char *string)
{
  ...
  while (len--)
    {
      if (g_ascii_isspace ((unsigned char) string[len]))
        string[len] = '\0';
      else
        break;
    }
...
g_strchug (char *string)
{
  ...
  for (start = (unsigned char*) string; *start && g_ascii_isspace (*start); start++)
    ;
...

{
  ...
  for (p = str; ((p - str) < max_len) && *p; p++)
      ...
      else 
	{
	  ...
	  last = p;
	  if (*(unsigned char *)p < 0xe0) /* 110xxxxx */
	    {
	      if (max_len - (p - str) < 2)
		goto error;
	    ...
	    {
	      if (*(unsigned char *)p < 0xf0) /* 1110xxxx */
		{
		  if ( max_len - (p - str) < 3)
		    goto error;
	      ...
	      else if (*(unsigned char *)p < 0xf5) /* 11110xxx excluding out-of-range */
		{
		  if ( (max_len - (p - str) < 4))
		    goto error;
...

{
  ...
  for (p = str; *p; p++)
      ...
      else 
	{
	  ...
	  if (*(unsigned char *)p < 0xe0) /* 110xxxxx */
	    {
	      if (G_UNLIKELY (*(unsigned char *)p < 0xc2))
		goto error;
	    }
	  else
	    {
	      ...
	      else if (*(unsigned char *)p < 0xf5) /* 11110xxx excluding out-of-range */
		{
		  switch (*(unsigned char *)p++ & 0x07)
		    ...
		    case 0:
		      VALIDATE_BYTE(0xc0, 0x80); /* 10xxxxxx */
		      if (G_UNLIKELY((*(unsigned char *)p & 0x30) == 0))
			goto error;
		      break;
		    case 4:
		      VALIDATE_BYTE(0xf0, 0x80); /* 0x80 ... 0x8f */
